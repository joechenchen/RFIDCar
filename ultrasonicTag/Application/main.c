...
uint8_t const ParaSet[16]={0xe7,0xe7,0xe7,0xe7,0xe7,79,5,0,0x61,0,0,0x40,0xfe,0,0,0};	//Ä¬ÈÏ²ÎÊı
//[0]~[6] read-only
//[7]~[15] can write
...
//								if(payload[5]!=0xff) b2=1;
					...
					else if(run_data_channel == radio_channel)
					{
						...
						{
							if(0xfe == CurParaSet[12])//½ÓÊÕËùÓĞµÄ³¬Éù²¨±êÇ©
							{
								...
								&&CurParaSet[15]==payload[5]))//Ã¿´Î¶¼Ö»½ÓÊÕÒ»¸ö±êÇ©£¬½ÓÊÕÆ¥ÅäµÄ³¬Éù²¨±êÇ©»òÕßËùÓĞµÄ³¬Éù²¨±êÇ©
							{
								if(0xff!=payload[16])
								{
									if(payload[16]>=0xfd)//ÎŞ³µÓÃ±êÇ©£¬Êä³öÔ¤ÉèÖµ
									{
									...
									b1=0;//485Êä³ö
									if(astRFID[0].USAge>=US_Leave_time)//µÚÒ»´ÎÊÕµ½³¬Éù²¨±êÇ©´¥·¢485Êä³ö
									{
										astRFID[0].USAge = 0;
										b1=1;
									}
									else
									{
										if(astRFID[0].USTagID[1] == payload[3]
											...
											&&astRFID[0].USTagID[3] == payload[5])//ÊÕµ½ÏàÍ¬µÄ³¬Éù²¨±êÇ©
										{
											astRFID[0].USAge = 0;
