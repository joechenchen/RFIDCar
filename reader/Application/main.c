...
//				payload[ii] = 0;
//			}
//			if(++ucCntRFRx>=RF_RX_FREQ) ucCntRFRx=0;
...
//			
//			radio_tx_carrier(RADIO_TXPOWER_TXPOWER_Pos4dBm,RADIO_MODE_MODE_Nrf_1Mbit,data_channel);
//			radio_channel = run_data_channel;
...
void radio_cmd_process(const uint8_t* RxPayload)
{
	uint8_t cmd;//ÃüÁî
...
//	rx_radio_power = (TxPaylod[24]&0x30)>>4;	
	...
	switch(cmd)
	{
		case 0://read
		{
			if(rx_page_addr<2&&rx_page_offset>15) error_flag=1;	//³¬³ö¶ÁÈ¡·¶Î§16Æ«ÒÆÁ¿
		...
		case 1://write
		{
			if(TxPaylod[13]&0x20) error_flag=1;	//ÅäÖÃ²ÎÊıÊ±£¬¼ÇÂ¼Æ«ÒÆÓĞĞ§Î»Îª0.
...
//						if(TxPaylod[27]&0x80)
//						{
//							TxPaylod[27]=CurParaSet[11]
//						}
					
				if(TxPaylod[28]==0xff||TxPaylod[28]<0xfc)
				{
					TxPaylod[28]=CurParaSet[5];
					TxPaylod[29]=CurParaSet[6];
					TxPaylod[30]=CurParaSet[7];
					TxPaylod[31]=CurParaSet[8];
				}
				else
				{
					if(TxPaylod[28]!=0xfd)
				...
				if(m<Rom_record_byte[rx_page_addr])//change
				{
					...
					TxPaylod[19]=0xE7;
					TxPaylod[20]=0xE7;
					TxPaylod[21]=CurParaSet[8];
					TxPaylod[22]=CurParaSet[9];
					TxPaylod[23]=CurParaSet[10];						
					nrf_nvmc_write_bytes(addr+ucROM0*Rom0_record_byte,&TxPaylod[16],16);	
					//½«ÔËĞĞ²ÎÊı±£´æÔÚacuSetÖĞ£¬ºóĞøÉèÖÃ²ÎÊı
					//ÉèÖÃÉúĞ§,[0]~[7] read-only
					for(m=8;m<16;m++) CurParaSet[m]=TxPaylod[m+9];//update parameter
					ucROM0++;
			...
			{					
				...
				{	//±È½ÏĞ´ÈëÄÚÈİÓë×î½ü¼ÇÂ¼£¬²»ÖØ¸´Ğ´Èë
					...
					{
						if(temp_flash1[m]!=TxPaylod[m])break;//different parameter
					}
		...
		case 4://read ram
		{
			for(m=0;m<16;m++) TxPaylod[m]=CurParaSet[m];
			ack = 4;
			break;
		}
		case 5://write ram
			...
			if(NRF_RADIO->CRCSTATUS)
			{
				...
				if(!radio_uc1)
				{
					...
					if(run_config_channel == radio_channel)
					{
						...
						else if((payload[1]&0x0a)==0x0a)//½ÓÊÕµ½ÅäÖÃĞÅÏ¢£¬¿ªÊ¼ÅäÖÃ²ÎÊı
						{
							b1=0;b2=0;
...
//											astRFID[rfid_uc2].RSSI= NRF_RADIO->RSSISAMPLE;
						...
						else if(work_mode == config_Tag)//
						{
							if(payload[2] == config.ID[0]&&payload[3] == config.ID[1]
								&&payload[4] == config.ID[2]&&payload[5] == config.ID[3])
							{
								if(0x80 == (payload[6]&0xff))//Ğ¯´ø½ÓÊÕ´°¿Ú?
								{
									//×ª·¢ÅäÖÃÖ¸Áî
									payload[1]=0x0a;
