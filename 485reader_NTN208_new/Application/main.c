...
uint8_t const ParaSet[16]={0xe7,0xe7,0xe7,0xe7,0xe7,79,5,0,0x71,0,0,0x40,0xfd,0,0,0x61};	//Ä¬ÈÏ²ÎÊı
//[0]~[6] read-only
//[7]~[15] can write
...
uint8_t caucID[4]={0xfe,0xfd,0x00,0x20};	//MSB first ½ÓÊÕÆ÷ID
...
//	uint8_t *temp_buf;
	...
	while(1)
	{
		if(uart_tx_en)
		{
			...
			tx_buf[20] = astRFID[0].USSta[1];//³¬Éù²¨ĞÅÏ¢
			 
			if(astRFID[0].carTagID[0] != 0xfd)//ÓĞ³µÔØ±êÇ©
			{
				...
				tx_buf[26] = astRFID[0].carSta[1];//³µÔØ±êÇ©
				pkt_len_temp = 20;
				tx_buf[27] = 0;
		...
		if(U_Master.MUART_RC_END_FLAG)
		{
			...
			if(!RX_PACKET.XOR)
			{
				...
				{
					#if 0
					if(caucID[0] == U_Master.Uart_RxBuffer[15]&&caucID[1] == U_Master.Uart_RxBuffer[16]
						&&caucID[2] == U_Master.Uart_RxBuffer[17]&&caucID[3] == U_Master.Uart_RxBuffer[18])
					{
						uart_tx_en = 1;
					}
					#endif
...
//				m |=0x20;//Æ«ÒÆÓĞĞ§
				...
				if(*pROM)
				{
					m = TxPaylod[13]&0xc0;//7~6Î»Ò³ÃæÖ¸Ê¾
					m |=0x20;//Æ«ÒÆÓĞĞ§
					TxPaylod[13] = m | set_page_offset;					
					addr = Page_Base[rx_page_addr];
					m = set_page_offset *Rom_record_byte[rx_page_addr];
				...
				else
				{
					m = TxPaylod[13]&0xc0;//7~6Î»Ò³ÃæÖ¸Ê¾
					m &=(~0x20);//Æ«ÒÆÎŞĞ§
					my_memset(&TxPaylod[16],Rom_record_byte[rx_page_addr]);
				}
				#endif
				if(0 == rx_page_addr)
				{
...
//								if(payload[5]!=0xff) b2=1;
					...
					else if(run_data_channel == radio_channel)
					{
						...
						{
							if(0xfe == CurParaSet[12] ||(0xfd == CurParaSet[12]
								&&CurParaSet[13]==payload[3]
								&&CurParaSet[14]==payload[4]
								&&CurParaSet[15]==payload[5]))
							{
								switch(rx_US)
								{
									case 0:
									...
									case 1:
										if(0 == payload[22])
										{
											astRFID[0].USTagID[0] = 0xff;
											astRFID[0].USTagID[1] = 0xff;;
											astRFID[0].USTagID[2] = 0xff;
											astRFID[0].USTagID[3] = 0xff;
											astRFID[0].USSta[0] = 0xff;
											astRFID[0].USSta[1] = 0xff;
											astRFID[0].carTagID[0] = 0xff;
											astRFID[0].carTagID[1] = 0xff;
											astRFID[0].carTagID[2] = 0xff;
											astRFID[0].carTagID[3] = 0xff;
											astRFID[0].carSta[0] = 0xff;
											astRFID[0].carSta[1] = 0xff;
											rx_US = 0;
										}
									break;
								}
